...
OUTPUT_FILE = "./original/sociology_added.txt"
...
# Main function to retrieve 600 sociology abstracts, 5322 sbstracts
def get_sociology_abstracts():
    query = "sociology[MeSH Terms] AND abstract[Title/Abstract]"
    total_abstracts = []
    total_results  = 70
    max_per_request = 70
...
sociology_abstracts = get_sociology_abstracts()
with open(OUTPUT_FILE, "w", encoding="utf-8") as original_file:
        for abstract in sociology_abstracts:
            original_file.write(abstract + "\n\n")

print(f"Total Sociology abstracts saved: {len(sociology_abstracts)}")
